...
class TestStorage(unittest.TestCase):
    ...
    def test_parse_sniper_value(self):
        ...
        # 2. 包含中文描述�?�?
        self.assertEqual(DatabaseManager._parse_sniper_value("建议�?100 元附近买�?), 100.0)
        self.assertEqual(DatabaseManager._parse_sniper_value("价格�?00.5�?), 100.5)
        
        # 3. 包含干扰数字（修复的Bug场景�?
        # 之前 "MA5" 会被错误提取�?5.0，现在应该提�?"�? 前面�?100
        text_bug = "无法给出。需等待MA5数据恢复，在股价回踩MA5且乖离率<2%时考虑100�?
        self.assertEqual(DatabaseManager._parse_sniper_value(text_bug), 100.0)
        
        # 4. 更多干扰场景
        text_complex = "MA10�?0.5，建议在30元买�?
        self.assertEqual(DatabaseManager._parse_sniper_value(text_complex), 30.0)
        
        text_multiple = "支撑�?0元，阻力�?0�? # 应该提取最后一�?�?前面的数字，�?0，或者更复杂的逻辑�?
        # 当前逻辑是找最后一个冒号，然后找之后的第一�?�?，提取中间的数字�?
        # 测试没有冒号的情�?
        self.assertEqual(DatabaseManager._parse_sniper_value("30�?), 30.0)
        
        # 测试多个数字�?�?之前
        self.assertEqual(DatabaseManager._parse_sniper_value("MA5 10 20�?), 20.0)
        
        # 5. Fallback: no "�? character �?extracts last non-MA number
