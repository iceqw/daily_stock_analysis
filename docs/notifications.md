...
| `NOTIFICATION_DEDUP_TTL_SECONDS` | advanced | noise | Variable or Secret | `3600` |
| `NOTIFICATION_COOLDOWN_SECONDS` | advanced | noise | Variable or Secret | `600` |
...
P4 降噪机制，只影响静态配置渠道，不影响 `send_to_context()` 的机器人触发会话回执。默认启用去重（1小时）和冷却（10分钟）。

| 配置 key | 默认值 | 说明 |
| --- | --- | --- |
| `NOTIFICATION_DEDUP_TTL_SECONDS` | `3600` | 同一稳定去重 key 在 TTL 内只发送一次；`0` 关闭 |
| `NOTIFICATION_COOLDOWN_SECONDS` | `600` | 同一冷却 key 在窗口内限频；`0` 关闭 |
...
- 去重 / 冷却状态的**内存热缓存**为当前 Python 进程内 dict，适用于 `main.py` 单进程和 `--serve` 单 worker。
- **SQLite 持久化层**（`notification_dedup_entries` 表）提供跨进程去重支持，Docker Compose 双服务模式下共享状态；DB 不可用时降级至内存模式（fail-open）。
...
- Pipeline 分发已收敛至 `NotificationService.send_with_results()` 统一入口，支持通过 `channel_content_overrides` 参数传入渠道级内容定制（如企业微信精简格式）。
