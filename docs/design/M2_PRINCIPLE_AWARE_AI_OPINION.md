...
The integration branch implements the v2 validator and server-controlled prompts, complete frozen Opinion snapshots with `ai_opinion_principle_refs`, additive/idempotent SQLite repair, existing Opinion detail response enrichment, and a reusable principle assessment UI component. This baseline has no accessible AI Opinion detail page, so page wiring is explicitly deferred to the next UI task; this PR does not add navigation or a large new page. Pending creation freezes active/current principles; generation consumes that stored snapshot; retry does not re-query principles; regenerate creates a new version. No automatic scheduling, trading block, RuleEngine, or user-controlled system prompt was added.
