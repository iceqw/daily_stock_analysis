...
The integration branch implements the v2 validator and server-controlled prompts, complete frozen Opinion snapshots with `ai_opinion_principle_refs`, additive/idempotent SQLite repair, existing Opinion detail response enrichment, and the minimal principle assessment UI component. Pending creation freezes active/current principles; generation consumes that stored snapshot; retry does not re-query principles; regenerate creates a new version. The local checkpoints are `0e267db`, `a1cf4cc`, and `2f46ace`. No automatic scheduling, trading block, RuleEngine, or user-controlled system prompt was added.
